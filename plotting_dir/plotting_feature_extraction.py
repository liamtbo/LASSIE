...
def find_force_drop_subranges(df: pd.DataFrame, percent_of_force_max: float):
    down_moves_subrange_list = []
    resistance_max = df['resistance'].max()
    min_drop_size = resistance_max * percent_of_force_max
    curr_peak_idx = 0
    curr_trough_idx = 0
...
def handle_depth_max(curve_data:pd.DataFrame, ax):
    resistance_at_depth_max = curve_data['resistance'].iloc[curve_data['depth'].values.argmax()]
    ax.plot([curve_data['depth'].max(),curve_data['depth'].max()], [0, resistance_at_depth_max], color='#D41159')

def handle_force_max(curve_data:pd.DataFrame, ax):
    depth_at_force_max = curve_data['depth'].iloc[curve_data['resistance'].values.argmax()]
    ax.plot([0,depth_at_force_max], [curve_data['resistance'].max(), curve_data['resistance'].max()], color='#D41159')

def handle_num_peaks(curve_data:pd.DataFrame, ax):
    subranges = find_force_drop_subranges(curve_data, percent_of_force_max=0.01)
...
def handle_force_mean(curve_data:pd.DataFrame, ax):
    mean = curve_data['resistance'].mean()
    res = curve_data['resistance']
    dep = curve_data['depth']
...
def handle_curve_2cm_slope(curve_data:pd.DataFrame, ax):
    ...
    dep = curve_data['depth']
    if dep.max() < 0.02:
        ax.plot([0,dep.max()], [0, res.max()], color='red')
    else:
        data_above_2cm = curve_data[curve_data['depth'] >= 0.02]
        ax.plot([0,data_above_2cm['depth'].iloc[0]], 
                [0, data_above_2cm['resistance'.iloc[0]]], color='red')

def handle_curve_overall_slope(curve_data:pd.DataFrame, ax):
    ax.plot([0,curve_data['depth'].max()], [0,curve_data['resistance'].max()], color='red')
...
def plot_feature_selection(feature_names:List[str], curves_data:List[pd.DataFrame], plot_idx:int):
    ...
    gloabl_depth_max = all_depth_resistance_data['depth'].max()
    gloabl_force_max = all_depth_resistance_data['resistance'].max()
...
def plot_feature_selection_seperately(feature_names: List[str], curves_data: List[pd.DataFrame], plot_idx: int):
    ...
    all_depth_resistance_data = pd.concat(curves_data, axis=0, ignore_index=True)
    global_depth_max = all_depth_resistance_data['depth'].max()
    global_force_max = all_depth_resistance_data['resistance'].max()

    curve_data = curves_data[plot_idx]

    for feature_name in feature_names:
        fig, ax = plt.subplots(figsize=(4, 4))  # one figure per feature
        ax.set_xlim([0, global_depth_max])
        ax.set_ylim([0, global_force_max])
        font_size = 10
        ax.set_xlabel('Depth (m)', fontsize=font_size)
        ax.set_ylabel('Resistance (N)', fontsize=font_size)
        # ax.set_title(feature_name.title(), fontsize=font_size)
