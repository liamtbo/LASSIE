...
ground = find_ground(pd.read_csv(filename))
...
def save_plots(df_list, filenames_list, save_plots_dst, save_bool=False, same_axis_range=True):
    if save_bool:
        ...
        for i, df in enumerate(df_list):
            ...
            plt.plot(df['depth'].to_nump, df['resistance'], marker='o', linestyle='-')
            plt.xlabel('Depth (m)')
            ...
            plt.grid(True)
            plt.savefig(f'{save_plots_dst}/{os.path.basename(filenames_list[i][:-3])}') # sliceing by -3 gets rid of extra '.csv' in filename
            plt.close()

save_plots(df_list, filenames_list, 'figures', save_bool=True, same_axis_range=False)
